...
export default new Event('channelDelete', async (guildChannel: NonThreadGuildBasedChannel) => {
    ...
    for (let l = 0; l < permGroups.length; l++) {
        ...
        if (permGroups[l].type == "role") {
            let role = guildChannel.guild.roles.resolve(permGroups[l]._id)
    ...
    server.sendLog('channelDelete', {embeds: [embed.embed]}, null, null)
});
