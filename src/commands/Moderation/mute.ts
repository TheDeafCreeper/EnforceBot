...
export default new Command({
    ...
    run: async ({ interaction, args }) => {
        ...
        let message = `<@!${target._id}> has been muted for ${reason}.`
        let DM = `You have been muted for ${reason}.`
        ...
        let part2 = ''
        if (duration != -1) part2 += ` The mute will expire <t:${toSec(Date.now() + duration)}:f>`
        else part2 += ` The mute will never expire.`
