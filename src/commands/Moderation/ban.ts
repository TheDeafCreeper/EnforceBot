...
export default new Command({
    ...
    run: async ({ interaction, args }) => {
        ...
        let message = `<@!${target._id}> has been banned for ${reason}.`
        let DM = `You have been banned for ${reason}.`
        ...
        let part2 = ''
        if (duration != -1) part2 += ` The ban will expire <t:${toSec(Date.now() + duration)}:f>`
        else part2 += ` The ban will never expire.`
